  ...
  <div className='shadow-md hover:shadow-lg transition-shadow overflow-hidden rounded-lg w-full sm:w-[330px]'>
    <Link to={`/listing/${listing._id}`}>
      ...
      <div className='p-3 flex flex-col gap-2 w-full'>
        <p className='truncate text-lg font-semibold text-grey-500'>{listing.name}</p>
        <div className='flex items-center gap-1'>
          <MdLocationOn className='h-4 w-4 text-green-700' />
          <p className='text-sm text-grey-500 truncate w-full'>{listing.address}</p>
        </div>
        <p className='text-sm text-gray-450 line-clamp-2'>{listing.description}</p>
        <p className='text-grey-500 mt-2 font-semibold '>
          Rs {' '}
